...
COIN3=SCN
COIN3l=scn
COINDAEMON=securecloud2d
COINDAEMONCLI=securecloud2-cli
COINCORE=.securecloud2
COINCONFIG=securecloud2.conf
COINHOME=/home/securecloud2
#wallet downnload and extractions commands
DOWNLOADCOINFILES=http://nullentry.com/chain/sc2/sc2.zip
COINFILES=sc2.zip
DECOMPRESS='unzip'
#rocketstrap
...
Test_Pause(){
#removeable Pause for testing
  read -p "Testing Pause - Report if you see this - Press [Enter] key to continue..." fackEnterKey
}
Function_Show_Balance(){
  echo -e ${GREEN}"Displaying ${COINl} Balance" ${CLEAR}
          ${COINDAEMONCLI} -datadir=${COINHOMEMASTER} getbalance
}
...
#warp2
  ...
  #Display Main Menu
    CHOICE=$(dialog --clear \
                    ...
                    --title "Main Menu" \
                    --menu "Choose one of the following options:" \
                    $MM_HEIGHT $MM_WIDTH $MM_CHOICE_HEIGHT \
                    "${MM_MainMenuOptions[@]}" \
                    2>&1 >/dev/tty)
  # Declare variables for Function_Show_Main_Menu
  case $CHOICE in
    ...
    2)
        ...
        echo -e ${GREEN}"Staking Option not Enabled" ${CLEAR}
        ;;
    3)
        clear
        echo -e ${YELLOW} "Displaying all Balances..."
        Function_Show_Balance
        ;;
    4)
        echo "VPS Manager not implimented yet"
        ;;
    5)
        clear
        Function_Donations
...
#warppoint1
  ...
  rm -r /usr/local/bin/${COINDAEMONCLI}
  rm -r /usr/local/bin/${COINDAEMON}
  pause
  }
  ## stop ALL MN function
  stop_All_Nodes(){
...
#Add pre dialog, to set Swap Space Size for larger installations
...
Function_Download_Coinfiles(){
#Start Function to download and stage coinfiles
  cd ~
...
#  echo ${Install_Count}
  ...
  Function_Donations(){
  #set or move to secondary script
  #  bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/securecloud2/master/donations.sh)
  #  seperate and make pretty with colors
  #Donation address for scriptworks and main projects
  #color code to seperate them
      clear
      dialog --title 'Where to Donate' --pause "Direct Donations to the Secure Cloud Network (SCN) Project can be made to.. BTC address: need btc wallet LTC address: do they have a LTC wallet SCN address: need main project donation address" \
      "Donations to Sburns the Script Writer can be made to...    BTC address: 32FzghE1yUZRdDmCkj3bJ6vJyXxUVPKY93  LTC address: MUdDdVr4Az1dVw47uC4srJ31Ksi5SNkC7H  SCN address: Need new address for new chain " \
      "Donations are used to fund VPS for testing, and server cost for addnodes, and things yet implimented." \ 25 80
  }
Function_IP_Table_Check(){
